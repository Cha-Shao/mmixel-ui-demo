...
export const CustomCarousel = () => {
  ...
  const prevPage = () => {
    page - 1 < 0 ? setPage(images.length - 1) : setPage(prevPage => prevPage - 1)
  }
  return (
    <div>
      <Carousel forcePage={page} className="mb-4 rounded-lg">
        {images.map((image, i) => (
          <Image
            key={i}
            src={`/carousel${image}`} alt=""
            className="w-full object-cover"
            width={720} height={405} />
        ))}
      </Carousel>
      <div className="flex items-center">
        <Button square onClick={prevPage} className="shrink-0">{'<'}</Button>
        ...
        <Button square onClick={nextPage} className="shrink-0">{'>'}</Button>
      </div>
    </div>
  )
}
...
> dots只需提供一个容器，组件将会自动在容器内放入children

<br/>

<Carousel
  arrowLeft={<p className="absolute text-3xl select-none cursor-pointer bottom-6 left-[1.5rem]">⬅️</p>} 
  arrowRight={<p className="absolute text-3xl select-none cursor-pointer bottom-6 left-[4.5rem]">➡️</p>}
  dots={<span className="absolute bottom-6 right-6 flex bg-pink-400/50 rounded-md"/>}
  dotAction={<p>🥰</p>}
...
| forcePage  | `number`       | `undefined` | 强制轮播页(index, 从0开始) |
...
| dotAction  | `ReactElement` | `undefined` | 自定义控件的高亮指示器     |
